...
def load_Konzil_data(flag='training', height=128, width=1840, num=None):
    ...
    for img_name in img_files:
        ...
        # Edited by JC
        img = np.array(Image.fromarray(img).resize([int(height/img.shape[0]*img.shape[1]), height]))
        ##   
...
def load_Konzil_data_square(flag='training', side_length=None, num=None, style='RGB'):
    ...
    for img_name in img_files:
        ...
        # Edited by JC
        img = np.array(Image.fromarray(img).resize([int(side_length/img.shape[0]*img.shape[1]), side_length]))
        ##   
        print(img.shape)
        if style=='RGB':
...
def load_ICFHR2018_data_lines(height = None, width = None, num=None, style='RGB', valid_rate = 0.2):
    if style=='RGB':
        dir_path = os.path.join(ROOT_FOLDER, 'icfhr2018_color')
        img_files = glob(os.path.join(dir_path,'train_data/*/*/*.jpg'))
    elif style=='gray':
        dir_path = os.path.join(ROOT_FOLDER, 'icfhr2018_gray_general_data')
        img_files = glob(os.path.join(dir_path,'train_data/*.jpg'))
    ...
    for img_name in train_img_files:
        ...
        if style=='RGB':
            if img.shape[1]<width:
                img = np.pad(img, [[0,0],[0, int(width-img.shape[1])],[0,0]], mode='constant', constant_values=255)
            else:
                img = img[:,:width, :]
            img = np.reshape(img, [1, height, width, 3])
        elif style=='gray':
            if img.shape[1]<width:
                img = np.pad(img, [[0,0],[0, int(width-img.shape[1])]], mode='constant', constant_values=255)
            else:
                img = img[:,:width]
...
def preprocess_ICFHR2018_lines_gray():
    x_train, x_test = load_ICFHR2018_data_lines(128, 2048, style='gray')
    np.save(os.path.join('data','icfhr2018_gray_general_data','train.npy'), x_train)
    np.save(os.path.join('data','icfhr2018_gray_general_data','test.npy'), x_test)
...
if __name__ == '__main__':
    ...
    preprocess_ICFHR2018_lines_gray()
