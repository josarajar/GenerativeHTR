...
def multi_normal(loc, log_scale):
    ...
    return tfd.Independent(distribution=tfd.Normal(loc=loc, scale=tf.exp(log_scale)), reinterpreted_batch_ndims=1)

def variable_summaries(var, name, summary_list):
  '''
...
class TwoStageVaeModelTF(object):
    ...
    def __build_loss(self):
        ...
        ### Edited by JC
        self.kl_loss1 = self.variational_posterior_z.kl_divergence(self.prior_z)
        self.kl_loss1.shape.assert_is_compatible_with([self.batch_size])
        self.kl_loss1 = tf.reduce_mean(self.kl_loss1)
        ##############
        ...
        self.kl_loss2 = self.variational_posterior_u.kl_divergence(self.prior_u)
        self.kl_loss2.shape.assert_is_compatible_with([self.batch_size])
        self.kl_loss2 = tf.reduce_mean(self.kl_loss2)
        ##############
    ...
    def __build_summary(self):
        with tf.name_scope('stage1_summary'):
            ...
            ## Edited by JC
            variable_summaries(self.mu_z, 'mu_z', self.summary1)
            variable_summaries(self.sd_z, 'sd_z', self.summary1)
            variable_summaries(self.logsd_z, 'logsd_z', self.summary1)
            #####
    ...
    def make_prior_u(self):
        # Zero mean, unit variance prior.
        self.prior_u_mean = tf.zeros(shape=(self.batch_size, self.latent_dim), dtype=tf.float32)
        self.prior_u_log_scale = tf.zeros(shape=(self.batch_size, self.latent_dim), dtype=tf.float32)
  
        self.prior_u = multi_normal(self.prior_u_mean, self.prior_u_log_scale)
    ...
    def build_encoder2(self):
        with tf.variable_scope('encoder'):
            ...
            # Edited by JC
            self.logsd_u = tf.log(tf.exp(self.logsd_u)+1)
            
            
            # Edited by JC
            self.variational_posterior_u = multi_normal(loc=self.mu_u, log_scale=self.logsd_u)
            #self.u = self.mu_u + self.sd_u * tf.random_normal([self.batch_size, self.latent_dim])
            self.u = self.variational_posterior_u.sample()
            ##
...
class Infogan(TwoStageVaeModelTF):
    def __init__(self, x, latent_dim=64, second_depth=3, second_dim=1024, cross_entropy_loss=False):
...
class Wae(TwoStageVaeModelTF):
    ...
    def build_encoder1(self):
        with tf.variable_scope('encoder'):
            ...
            # Edited by JC
            y = tf.nn.relu(batch_norm(tf.layers.conv2d(y, 1024, 5, 2, 'same'), self.is_training, 'bn5'))
...
class Resnet(TwoStageVaeModelTF):
...
# Added by JC           
class Wae_rect(TwoStageVaeModelTF):
    def __init__(self, x, latent_dim=64, second_depth=3, second_dim=1024, cross_entropy_loss=False):
        super(Wae_rect, self).__init__(x, latent_dim, second_depth, second_dim, cross_entropy_loss)

    def make_prior_z(self):
        # Zero mean, unit variance prior.
        self.prior_z_mean = tf.zeros(shape=(self.batch_size, self.latent_dim), dtype=tf.float32)
        self.prior_z_log_scale = tf.zeros(shape=(self.batch_size, self.latent_dim), dtype=tf.float32)
  
        self.prior_z = multi_normal(self.prior_z_mean, self.prior_z_log_scale)
    ...
    def build_encoder1(self):
        with tf.variable_scope('encoder'):
            ...
            self.logsd_z = tf.layers.dense(y, self.latent_dim)
            self.sd_z = tf.exp(self.logsd_z)
            # Edited by JC (reparametrization)
            self.logsd_z = tf.log(tf.exp(self.logsd_z)+1)
            
            # Edited by JC
            self.variational_posterior_z = multi_normal(loc=self.mu_z, log_scale=self.logsd_z)
            #self.z = self.mu_z + tf.random_normal([self.batch_size, self.latent_dim]) * self.sd_z 
            self.z = self.variational_posterior_z.sample()
            ##
